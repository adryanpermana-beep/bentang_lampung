...
class ProduksiHasilHutanController extends Controller
{
    /**
     * Mengambil GeoJSON Hasil Hutan per Desa/Kelurahan dengan Gradasi Warna Dinamis
     */
    public function getGeojson($kategori = null, $kode_kecamatan = null)
    {
        ...
        try {
            // 3. Subquery agregasi detail komoditas hasil hutan per desa
            $subQueryDetail = "
                SELECT 
                ...
                FROM produksi.hasil_hutan
                GROUP BY kode_desa::text
            ";

            // 4. Query Utama PostGIS dengan Logika Gradasi Warna Dinamis (Gradien Cokelat Hutan)
            $sqlFeature = "
                SELECT json_build_object(
                    ...
                    'properties', json_build_object(
                        ...
                        -- Logika Warna Dinamis Berdasarkan Tingkat Hasil Panen
                        'theme_color', CASE 
                            WHEN COALESCE(dt.sum_produksi, 0) = 0 THEN '#CBD5E1'   -- Abu-abu (Tidak Ada Data/Produksi 0)
                            WHEN dt.sum_produksi < 10 THEN '#D97706'               -- Cokelat Terang / Amber (Rendah)
                            WHEN dt.sum_produksi < 100 THEN '#B45309'              -- Cokelat Sedang (Sedang)
                            WHEN dt.sum_produksi < 1000 THEN '#8B5A2B'             -- Cokelat Kayu Hutan (Tinggi)
                            ELSE '#4A2E11'                                         -- Cokelat Pekat Tua (Sangat Tinggi)
                        END,
                        'border_color', '#1E293B',
                        'hover_color', '#F59E0B'
                    )
