...
class ProduksiBuahBuahanController extends Controller
{
    /**
     * Mengambil GeoJSON Buah-Buahan per Desa/Kelurahan
     */
    public function getGeojson($kategori = null, $kode_kecamatan = null)
    {
        ...
        // 1. Tangkap kode_kecamatan dari berbagai opsi parameter input
        $targetKode = null;
        ...
        try {
            // 3. Subquery agregasi detail komoditas buah-buahan per desa
            $subQueryDetail = "
                SELECT 
                ...
                FROM produksi.buah_buahan
                GROUP BY kode_desa::text
            ";

            // 4. Query Utama PostGIS untuk menggabungkan wilayah & data produksi
            $sqlFeature = "
            ...
            // 5. Bungkus menjadi struktur FeatureCollection
            $sqlCollection = "
                SELECT json_build_object(
                    'type', 'FeatureCollection',
